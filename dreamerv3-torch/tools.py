...
class Logger:
    def __init__(self, logdir, step):
        ...
        self._images = {}
        self._videos = {}
        self._can_video = self._check_video_support()
        self.step = step

    def _check_video_support(self):
        # torch.utils.tensorboard's add_video depends on moviepy.editor; when it's missing,
        # video logging becomes a silent no-op. Fallback to logging frame strips as images.
        try:
    ...
    def write(self, fps=False, step=False):
        ...
        for name, value in self._videos.items():
            ...
            if self._can_video:
                ...
                # Fallback: log a horizontal strip of evenly spaced frames (first batch item).
                frames = value[0]  # (T, H, W, C)
                num = min(8, T)
                if num <= 1:
                    strip = frames[0]
                else:
                    idxs = np.linspace(0, T - 1, num=num, dtype=np.int32)
                    strip = np.concatenate([frames[i] for i in idxs], axis=1)
                self._writer.add_image(name + "_strip", strip, step, dataformats="HWC")
...
def simulate(
    ...
    def _is_joiner_env(env):
        try:
            return bool(getattr(env, "is_joiner"))
        except Exception:
            return False

    skip_envs = np.array([_is_joiner_env(e) for e in envs], dtype=bool)
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if now - last_heartbeat > 10.0:
            ...
            for e in envs:
                try:
                    if hasattr(e, "_env") and hasattr(e._env, "detect_state"):
                        img = e._env.capture_screen()
                        env_states.append(e._env.detect_state(img))
                    else:
                        env_states.append("unknown")
                except Exception:
                    env_states.append("error")
            print(f"[heartbeat] step={step} done={done.tolist()} states={env_states}")
            last_heartbeat = now
        ...
        if done.any():
            ...
            for index, result in zip(indices, results):
                ...
                t["reward"] = 0.0
                t["discount"] = 1.0
                # initial state should be added to cache (host only)
                if not skip_envs[index]:
        ...
        episode += int((done & ~skip_envs).sum())
        length += 1
        step += len(envs)
        length *= 1 - done
        # add to cache
        for i, (a, result, env) in enumerate(zip(action, results, envs)):
            if skip_envs[i]:
                continue
            o, r, d, info = result
            ...
            if isinstance(a, dict):
                transition.update(a)
                if "action" in info:
                    transition["action"] = info["action"]
            else:
                transition["action"] = info.get("action", a)
            transition["reward"] = r
