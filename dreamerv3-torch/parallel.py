...
class Worker:
    ...
    def __init__(self, fn, strategy="thread", state=False):
        ...
        self.impl = {
            ...
            "thread": bind(ThreadWorker, initializers=inits),
        }[strategy](fn)
        self.promise = None
...
class ThreadWorker:
    def __init__(self, fn, initializers=()):
        ...
        self._thread = threading.Thread(
            target=self._loop, args=(self._in_queue, self._out_queue, fn, initializers), daemon=True
        )
        self._thread.start()
        self._nextid = 0
        self._results = {}
    ...
    def close(self):
        self._in_queue.put((Message.STOP, self._nextid, None))

    def _submit(self, message, payload=None):
        callid = self._nextid
        self._nextid += 1
        self._in_queue.put((message, callid, payload))
        return Future(self._receive, callid)

    def _receive(self, callid):
        while callid not in self._results:
            message, cid, payload = self._out_queue.get()
    ...
    @staticmethod
    def _loop(in_q, out_q, function, initializers):
        try:
            state = None
            [fn() for fn in initializers]
            while True:
                message, callid, payload = in_q.get()
                if message == Message.OK:
                    out_q.put((Message.RESULT, callid, True))
                elif message == Message.STOP:
                    return
                elif message == Message.RUN:
                    args, kwargs = payload
                    state, result = function(state, *args, **kwargs)
                    out_q.put((Message.RESULT, callid, result))
        except Exception:
            stacktrace = "".join(traceback.format_exception(*sys.exc_info()))
            print(f"Error inside thread worker: {stacktrace}.", flush=True)
            out_q.put((Message.ERROR, callid, stacktrace))
